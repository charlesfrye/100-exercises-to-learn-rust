    ...
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(TicketNewError::DescriptionError { .. }) => {
            match Ticket::new(title, "Description not provided".into(), status) {
                Ok(ticket) => ticket,
                Err(TicketNewError::TitleError { details }) => panic!("{details}"),
                Err(TicketNewError::DescriptionError { .. }) => panic!(),
            }
        }
        Err(TicketNewError::TitleError { details }) => panic!("{details}"),
    }
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {
                details: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        ...
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError {
                details: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
